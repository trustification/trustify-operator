...
public class ServerConfigurationConfigMapDiscriminator implements ResourceDiscriminator<ConfigMap, Trustify> {
    @Override
    public Optional<ConfigMap> distinguish(Class<ConfigMap> resource, Trustify cr, Context<Trustify> context) {
        String configMap = ServerConfigurationConfigMap.getConfigMapName(cr);
